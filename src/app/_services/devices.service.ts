...
export class DevicesService {
    ...
    public getDevices(): Observable<Array<Device>> {
        return this.http.get<Array<Device>>('/auth/api/v1/get_devices').map((payload: any) => {
    ...
    public deleteDevices(device_id: string, keep_data?: string): Observable<Array<Device>> {
        ...
        return this.http.delete<any>('/auth/api/v1/delete_device', options);
    }

    public addDevices(devices_name: string): Observable<any> {
        return this.http.post<any>('/auth/api/v1/register_device', {devices_name});
    }
}
